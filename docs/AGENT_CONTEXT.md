...
- **Story UI:** Full-screen **story player** (Phase E) after **Start Wrapped** in lobby. Lobby shows lede + parse warnings with layman slide impact (gated Start). Player: IG-style progress segments, tap left/right, hold-to-pause, auto-advance (last slide waits for tap), exit via × / Escape / swipe-down. Nav hidden in player only. Desktop: full-width backdrop, centered 9:16 card. Sharing: screenshot hint on player.
- **Slides:** intro → activity span → activity → likes → comments → story interactions → DMs → profile searches → privacy → feed personality. Leaderboards capped at `WRAPPED_SOCIAL_LEADERBOARD_LIMIT` (4).
...
- **Phase E (story player):** `WrappedLobby.jsx`, `WrappedStoryPlayer.jsx`, `WrappedPlayerContext.jsx`, `parseWarningImpact.js`, `config/wrappedPlayer.js` — lobby + immersive player; replaces vertical scroller deck for loaded exports.
