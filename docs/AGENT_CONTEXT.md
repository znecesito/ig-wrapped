...
- **Story UI:** Ten **9:16** cards in a vertical scroller (scroll-snap, Prev/Next, dots, keyboard). **Save slide** rasterizes a hidden **1080×1920 export frame** (`WrappedSlideExport.jsx`, export-only CSS, full `@` labels) — not the interactive scroller card. Delivery via `wrappedCardCapture.js` + `saveWrappedCardImage.js` (Web Share / download / preview fallback).
