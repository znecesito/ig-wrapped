...
- **Story UI:** Ten **9:16** cards in a vertical scroller (scroll-snap, Prev/Next, dots, keyboard). Card innards use **container queries** (`container-type: inline-size` on `.wrapped-card`, `cqw` for type/charts) so **Save slide** at 1080×1920 scales like the on-screen card. Export: `wrappedCardCapture.js` + `saveWrappedCardImage.js` (Web Share / download / preview fallback). Components: `WrappedPage.jsx`, `wrappedSlideContent.jsx`, `WrappedSlideChrome.jsx`, `WrappedAvatarPodium.jsx`; themes/palette/avatars in `wrappedThemes.js`, `wrappedPalette.js`, `wrappedAvatars.js`.
